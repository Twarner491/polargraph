...
function generateGcodeForEntities(entities, colorId = null) {
    ...
    const settings = {
        ...
        feed_rate_travel: parseFloat(document.getElementById('feedTravel')?.value) || 3000,
        feed_rate_draw: parseFloat(document.getElementById('feedDraw')?.value) || 1500,
...
async function generatePattern() {
    ...
    elements.generatorOptions.querySelectorAll('input, select, textarea').forEach(input => {
    ...
    if (generator === 'gpent') {
        try {
            ...
            // Log messages from GPenT
            if (result.logs) {
                result.logs.forEach(log => logConsole(log, 'msg-info'));
            }
            ...
            // Server mode returns pre-generated entities
                ...
                logConsole(`GPenT created ${result.entities.length} elements`, 'msg-info');
...
function applySettingsToUI(settings) {
    ...
    document.getElementById('feedTravel').value = settings.feed_rate_travel || 3000;
    document.getElementById('feedDraw').value = settings.feed_rate_draw || 1500;
