...
class GCodeGenerator {
    ...
    turtleToGcode(turtle) {
        ...
        // Header - standards-compliant, compatible with G-code viewers
        gcode.push('; Generated by Polargraph (Client-side)');
        gcode.push('; Makelangelo-compatible G-code');
        gcode.push(`; Feed rate travel: ${this.settings.feed_rate_travel} mm/min`);
        gcode.push(`; Feed rate draw: ${this.settings.feed_rate_draw} mm/min`);
        gcode.push('');
        gcode.push('G21 ; Units: millimeters');
        gcode.push('G90 ; Absolute positioning');
        gcode.push(`G0 F${this.settings.feed_rate_travel} ; Set travel speed`);
        gcode.push(`G1 F${this.settings.feed_rate_draw} ; Set draw speed`);
        ...
        // Footer - lift pen, then return home
        gcode.push('');
